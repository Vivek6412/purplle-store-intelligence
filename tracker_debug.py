...
class ReIDTracker:
    ...
    def __init__(self, sim_threshold: float = 0.85, gallery_size: int = 8):
    ...
    def resolve_visitor(self, track_id: int, embedding: torch.Tensor) -> tuple[str, bool]:
...
class StaffClassifier:
    """Detects staff wearing solid black tops and black pants using spatial-color uniformity."""
    ...
    def is_staff(self, frame: np.ndarray, bbox: list[float], bbox_area: float) -> bool:
        ...
        try:
            ...
            # 2. Extract upper torso and lower legs segments to check full black dress code
            # We skip the very bottom to avoid picking up the floor or shelf colors!
            torso_y1, torso_y2 = int(h * 0.20), int(h * 0.40)
            legs_y1, legs_y2 = int(h * 0.60), int(h * 0.80)

            torso = normalized_bgr[torso_y1:torso_y2, :]
            legs = normalized_bgr[legs_y1:legs_y2, :]
            ...
            def evaluate_black_fabric(region: np.ndarray) -> bool:
                ...
                # Normalized solid black fabric thresholds (very strict)
                ...
                return ratio_black > 0.65 and v_std < 30.0 and s_std < 25.0

            def evaluate_pink_fabric(region: np.ndarray) -> bool:
                hsv = cv2.cvtColor(region, cv2.COLOR_BGR2HSV)
                h, s, v = cv2.split(hsv)
                # Pink/Red hue in OpenCV HSV
            ...
            # Both torso AND legs must match the strict uniform code to be classified as staff
            # Now allowing Pink Top + Black Pants, or Black Top + Black Pants
            return (is_torso_black or is_torso_pink) and is_legs_black
...
class VisitorStateTracker:
    ...
    def update(
        ...
        print(f'CROSSING DIRECTION: {crossing_direction}')
...
class TrackingPipeline:
    ...
    def __init__(self, store_layout: dict, reid_threshold: float = 0.85):
    ...
    def process_frame(
        ...
        for track_id, bbox, confidence in detections:
            ...
            visitor_id, is_reentry = self.reid_tracker.resolve_visitor(track_id, embedding)

            bbox_area = (bbox[2] - bbox[0]) * (bbox[3] - bbox[1])
            is_staff = self.staff_classifier.is_staff(frame, bbox, bbox_area)
